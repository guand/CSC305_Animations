...
void main() {
    ...
    float weight_tot = 0;
    color = texture(tex, vec2(gl_FragCoord)/tex_width).rgb * weight[0];
    for(int i = 1; i < 5; i++) {
        color += texture(tex, (vec2(gl_FragCoord)+vec2(0.0, offset[i])/tex_width)).rgb * weight[i];
        color += texture(tex, (vec2(gl_FragCoord)-vec2(0.0, offset[i])/tex_height)).rgb * weight[i];
    }
